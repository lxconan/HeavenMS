...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.tryacquireClient()) { // thanks MedicOP for assisting on concurrency protection here
            try {
                World wserv = WorldServer.getInstance().getWorld(c.getWorld());
                if(wserv == null) {
                ...
                */
                ...
                if (player.getGuildId() > 0) {
                    ...
                    if (playerGuild == null) {
                        ...
                        GuildAndAllianceService.getInstance().setGuildMemberOnline(player, true, c.getChannel());
                ...
                if (newcomer) {
                    ...
                    */
                    if (player.isGM()){
                        WorldServer.getInstance().broadcastGMMessage(c.getWorld(), MaplePacketCreator.earnTitleMessage((player.gmLevel() < 6 ? "GM " : "Admin ") + player.getName() + " has logged in"));
                    }
