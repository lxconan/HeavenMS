...
public class MapleMonsterAggroCoordinator {
    ...
    private long lastStopTime = ServerTimer.getInstance().getCurrentTime();
    ...
    public void stopAggroCoordinator() {
        ...
        lastStopTime = ServerTimer.getInstance().getCurrentTime();
    }
    ...
    public void startAggroCoordinator() {
        ...
        int timeDelta = (int) Math.ceil((ServerTimer.getInstance().getCurrentTime() - lastStopTime) / YamlConfig.config.server.MOB_STATUS_AGGRO_INTERVAL);
        if (timeDelta > 0) {
