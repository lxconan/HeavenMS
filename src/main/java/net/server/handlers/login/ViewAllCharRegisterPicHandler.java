...
public final class ViewAllCharRegisterPicHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        int channel = Randomizer.rand(1, WorldServer.getInstance().getWorld(c.getWorld()).getChannelsSize());
        c.setChannel(channel);

        String pic = slea.readMapleAsciiString();
        c.setPic(pic);

        String[] socket = WorldServer.getInstance().getInetSocket(c.getWorld(), channel);
        if (socket == null) {
            c.announce(MaplePacketCreator.getAfterLoginError(10));
            return;
        }
