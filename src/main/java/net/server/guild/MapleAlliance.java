...
public class MapleAlliance {
        ...
        if(alliance != null) {
            ...
            try {
                for(int i = 0; i < guildMasters.size(); i++) {
                    ...
                    GuildAndAllianceService.getInstance().getGuild(chr.getGuildId()).getMGC(chr.getId()).setAllianceRank((i == 0) ? 1 : 2);
                    chr.saveGuildStatus();
                }
    ...
    public static boolean removeGuildFromAlliance(int allianceId, int guildId, int worldId) {
        ...
        MapleAlliance alliance = GuildAndAllianceService.getInstance().getAlliance(allianceId);
    ...
    public static void sendInvitation(MapleClient c, String targetGuildName, int allianceId) {
        MapleGuild mg = GuildAndAllianceService.getInstance().getGuildByName(targetGuildName);
        if(mg == null) {
