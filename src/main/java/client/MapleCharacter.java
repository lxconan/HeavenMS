...
public class MapleCharacter extends AbstractMapleCharacterObject {
    ...
    public void debugListAllBuffs() {
        ...
        try {
            logger.debug("-------------------");
            logger.debug("CACHED BUFF COUNT: ");
            for(Entry<MapleBuffStat, Byte> bpl : buffEffectsCount.entrySet()) {
                logger.debug(bpl.getKey() + ": " + bpl.getValue());
            }

            logger.debug("-------------------");
            logger.debug("CACHED BUFFS: ");
            for(Entry<Integer, Map<MapleBuffStat, MapleBuffStatValueHolder>> bpl : buffEffects.entrySet()) {
                logger.debug(bpl.getKey() + ": ");
                for(Entry<MapleBuffStat, MapleBuffStatValueHolder> pble : bpl.getValue().entrySet()) {
                    logger.debug(pble.getKey().name() + pble.getValue().value + ", ");
                }
            }
            logger.debug("-------------------");
            logger.debug("IN ACTION:");
            for(Entry<MapleBuffStat, MapleBuffStatValueHolder> bpl : effects.entrySet()) {
                logger.debug(bpl.getKey().name() + " -> " + MapleItemInformationProvider.getInstance().getName(bpl.getValue().effect.getSourceId()));
            }
        } finally {
            chrLock.unlock();
            effLock.unlock();
    ...
    public void debugListAllBuffsCount() {
        ...
        try {
            for(Entry<MapleBuffStat, Byte> mbsl : buffEffectsCount.entrySet()) {
                logger.debug(mbsl.getKey().name() + " -> " + mbsl.getValue());
            }
        } finally {
            chrLock.unlock();
            effLock.unlock();
    ...
    public void sendPolice(String text) {
        ...
        if (WorldServer.getInstance().isGmOnline(this.getWorld())) { //Alert and log if a GM is online
            WorldServer.getInstance().broadcastGMMessage(this.getWorld(), MaplePacketCreator.sendYellowTip(message));
    ...
    public void autoban(String reason) {
        ...
        WorldServer.getInstance().broadcastGMMessage(this.getWorld(), MaplePacketCreator.serverNotice(6, MapleCharacter.makeMapleReadable(this.name) + " was autobanned for " + reason));
    }
